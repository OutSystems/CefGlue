...
namespace Xilium.CefGlue.Common.Shared.Serialization.Json
{
    internal class CefJsonDeserializer : IDeserializer
    {
        ...
        private static Utf8JsonReader CreateJsonReader(byte[] bytes)
        {
            return new Utf8JsonReader(bytes, new JsonReaderOptions { MaxDepth = DeserializerMaxDepth });
        }

        private void Deserialize(Utf8JsonReader reader, params IDeserializerState[] initialStates)
        {
            var state = new Stack<IDeserializerState>(initialStates);
        ...
        private object ReadComplexObject(ref Utf8JsonReader reader, Stack<IDeserializerState> state, IDictionary<string, object> referencesMap)
        {
